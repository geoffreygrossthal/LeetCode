...
//Program: Leetcode problem Kids With the Greatest Number of Candies
...
public class KidsWithTheGreatestNumberOfCandies {
    ...
    //Method returns a boolean list, if an index is true means the kid will have the most candies given the extra
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {

        int most_candies = candies[0];
        List<Boolean> list = new ArrayList<>();
        
        for (int i = 0; i < candies.length; ++i) {
            if (most_candies < candies[i])
