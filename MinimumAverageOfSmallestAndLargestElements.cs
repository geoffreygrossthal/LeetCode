...
public class MinimumAverageOfSmallestAndLargestElements {
    ...
    //Method returns the minimum average of the array
    public double MinimumAverage(int[] nums) {
        Array.Sort(nums);
        double current_average;
        double min_average = double.MaxValue;;
        for (int i = 0; i < nums.Length - i; ++i) {
            current_average = (double)(nums[i] + nums[nums.Length - (i + 1)]) / 2;
