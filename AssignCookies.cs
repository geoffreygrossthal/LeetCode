...
//Program: Leetcode problem Assign Cookies
...
public class AssignCookies {
    ...
    //Method returns the maximum distribution of cookies
    public int FindContentChildren(int[] g, int[] s) {
        //Number of assigned cookies
        int assgined_cookies = 0;
        //Sort arrays
        Array.Sort(g);
        Array.Sort(s);
        //Find accetable amount of cookies
        for (int i = 0; i < s.Length; ++i) {
            //No more children to assign cookies to
            if (assgined_cookies >= g.Length)
                return assgined_cookies;
            //Accetable cookie, increment
            else if (s[i] >= g[assgined_cookies])
                ++assgined_cookies;
        }
        return assgined_cookies;
    }
